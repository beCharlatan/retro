...
async function run() {
  ...
  await withBrowser(async (browser) => {
    ...
    // --- framing: its timers (default 0:40) are configurable too ---
    {
      const page = await openPage(browser, report);
      try {
        ...
        report.check(
          'framing: timer starts at the default 0:40',
          (await valueOf_(page, timer)) === '0:40',
        );
        await page.click(`${timer} button[aria-label="Увеличить время"]`);
        await page.click(`${timer} button[aria-label="Увеличить время"]`);
        report.check(
          'framing: two "+" presses add 2 × 5 s (0:50)',
          (await valueOf_(page, timer)) === '0:50',
        );
        report.check(
          'framing: "вернуть 0:40" is offered',
          (await page.textContent(`${timer} .timer-default`)).includes('0:40'),
        );
        await page.click(`${timer} .timer-default`);
        report.check('framing: "вернуть" restores 0:40', (await valueOf_(page, timer)) === '0:40');
      } catch (e) {
