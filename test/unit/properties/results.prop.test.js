...
describe('no results function ever produces NaN or Infinity', () => {
  const cases = [
    ...
    [
      ...
      rowsOf(
        fc.record({
          ...
          choice: nullable(fc.constantFrom('1', '2')),
        }),
      ),
      (d) => framingResults(d),
    ],
    ...
    [
      ...
      rowsOf(
        fc.record({
          r1Role: fc.constantFrom('owner', 'buyer'),
          r2Role: fc.constantFrom('owner', 'buyer'),
          r1Price: nullable(int(0, 1000)),
          r2Price: nullable(int(0, 1000)),
        }),
      ),
  ...
  test('anchoring / false consensus / framing / prisoner’s dilemma shares are 0..100', () => {
    ...
    fc.assert(
      fc.property(
        ...
        (d) => {
          const r = framingResults(d);
          expect(inRange(r.aRisky, 0, 100)).toBe(true);
          expect(inRange(r.bRisky, 0, 100)).toBe(true);
        },
      ),
      RUNS,
    );
    ...
    fc.assert(
      fc.property(
        ...
        ({ pct, n, avg, delta, ratio }) => {
          ...
          clean(REVEAL_COPY.endowment({ avgWTA: 100 * ratio, avgWTP: 100, ratio }));
          clean(
          ...
          clean(REVEAL_COPY.framing({ aRisky: pct, bRisky: Math.max(0, 100 - pct) }));
          clean(
