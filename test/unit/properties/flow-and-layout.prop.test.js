...
describe('map physics', () => {
  ...
  test('computeSafeBounds always leaves a playable rectangle that lies inside the window', () => {
    fc.assert(
      fc.property(
        ...
        (w, h) => {
          ...
          expect(b.maxX - b.minX).toBeGreaterThanOrEqual(120 - 1e-9);
          expect(b.maxY - b.minY).toBeGreaterThanOrEqual(120 - 1e-9);
  ...
  test('computeIconScale stays within [0.4, 1] and never grows as the window shrinks', () => {
    fc.assert(
      fc.property(
        ...
        int(1, 30),
        (w, h, count) => {
          const s = computeIconScale(computeSafeBounds(w, h), count);
          expect(s).toBeGreaterThanOrEqual(0.4);
          expect(s).toBeLessThanOrEqual(1);
          const smaller = computeIconScale(computeSafeBounds(w - 100, h - 100), count);
          expect(smaller).toBeLessThanOrEqual(s + 1e-9);
        },
      ),
      RUNS,
    );
