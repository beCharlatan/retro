...
async function run() {
  ...
  await withBrowser(async (browser) => {
    // --- pairs: Ультиматум ---
    {
      ...
      report.check('pairs: first click marks the person as selected',
        selectedClass && selectedClass.includes('swap-selected'), selectedClass);
      ...
      report.check('pairs: the two clicked people swapped positions',
        after[0] === before[2] && after[2] === before[0], `${before} -> ${after}`);
      report.check('pairs: everyone else stayed put',
      ...
      const beforeCancel = await page.$$eval('.role-pair-name', els => els.map(e => e.textContent));
      await page.click('.role-pair-name >> nth=1');
      await page.click('.role-pair-name >> nth=1');
      await page.waitForTimeout(80);
      const afterCancel = await page.$$eval('.role-pair-name', els => els.map(e => e.textContent));
      ...
      report.check('pairs: clicking the same person twice cancels (no swap)',
    ...
    // --- groups: Эффект владения ---
    {
      ...
      const aBefore = await page.$$eval('.role-group-a .role-chip', els => els.map(e => e.textContent));
      const bBefore = await page.$$eval('.role-group-b .role-chip', els => els.map(e => e.textContent));
      ...
      const aAfter = await page.$$eval('.role-group-a .role-chip', els => els.map(e => e.textContent));
      const bAfter = await page.$$eval('.role-group-b .role-chip', els => els.map(e => e.textContent));

      report.check('groups: sizes stay balanced after a swap',
        aAfter.length === aBefore.length && bAfter.length === bBefore.length,
        `A: ${aBefore.length}->${aAfter.length}, B: ${bBefore.length}->${bAfter.length}`);
      report.check('groups: the two clicked people actually swapped sides',
      ...
      const aBeforeNoop = await page.$$eval('.role-group-a .role-chip', els => els.map(e => e.textContent));
      await page.click('.role-group-a .role-chip >> nth=0');
      await page.click('.role-group-a .role-chip >> nth=1');
      await page.waitForTimeout(100);
      const aAfterNoop = await page.$$eval('.role-group-a .role-chip', els => els.map(e => e.textContent));
      report.check('groups: swapping within the same group is a no-op',
