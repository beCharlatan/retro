...
const { Report, openPage, withBrowser, DIST_URL } = require('./lib');
...
async function run() {
  ...
  await withBrowser(async (browser) => {
    ...
    // --- framing ---
    {
      ...
      report.check('framing: both group texts hidden by default', aHidden !== null && bHidden !== null);
      ...
      report.check('framing: copying group B works without revealing it',
        clipB.includes('400 человек'), clipB.slice(0, 40));
      ...
      report.check('framing: revealing group A does not reveal group B',
      ...
      report.check('framing: group A clipboard content matches group A wording',
        clipA.includes('200 человек') && !clipA.includes('400 человек'), clipA.slice(0, 40));
    ...
    // --- barnum ---
    {
      ...
      report.check('barnum: clipboard matches the profile text shown on screen',
        clip.startsWith('Иногда вы сомневаетесь'), clip.slice(0, 40));
